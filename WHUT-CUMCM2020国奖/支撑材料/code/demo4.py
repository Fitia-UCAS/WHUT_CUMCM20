...
def main(gaowengailv, ISREAD_MAZES, PATH):
    ...
    if ISREAD_MAZES:
        ...
        print("平均获得资金", sum(jiage)/huos, '活着概率', round(huos*100/learn_num,2),"%")
    else:
...
if __name__ == '__main__':
    ...
    for gaowengailv in np.arange(0,1-shabao,0.1):
        ...
        main(gaowengailv2, ISREAD_MAZES, 'demo4_gailv_1_01_{}'.format(int(gaowengailv * 100)))
    ...
    ax1.plot(x, huozhe, label="保守策略时存活率", linestyle='--', marker='*')
    ax1.plot(x, huozhe2, label="非保守策略时存活率", linestyle='--', marker='*')
