...
4.找出种群中np=0的个体，将其从种群中移除，将其加入到分层
集合fk中，该集合就是层级为0个体的集合。
5.判断fk是否为空，若不为空，将fk中所有的个体sp中对应的个
体层级减去1，且k=k+1,跳到2;
    ...
    for p in range(0, len(values1)):
        ...
        for q in range(0, len(values1)):
            ...
            if (
                ...
                # 支配判定条件:当且仅当,对于任取i属于{1,2},
                # 都有fi(p)>fi(q),符合支配.
                # 或者当且仅当对于任意i属于{1,2},有fi(p)>=
                # fi(q),且至少存在一个j使得fj(p)>fj(q)符合弱支配
                if q not in S[p]:
                    # 同时如果q不属于sp将其添加到sp中
                    S[p].append(q)
            # 如果q支配p
...
class NSGA(object):
    def __init__(
        self, loaders, partition_num=None, max_num=20, pop_size=10, c_rate=0.5, m_rate=0.8
    ):
        ...
        for i in range(gene.shape[1]):  # 交叉，这里待优化
            for j in range(gene.shape[0]):
                if np.random.rand() > 0.8:
                    newGene[j][i] = (
                        newGene[j][i] + random.randint(0, lingyu)
                        if newGene[j][i] + lingyu < self.loaders[i].num
                        else newGene[j][i]
                    )
                elif np.random.rand() < 0.2:
                    newGene[j][i] = (
                        newGene[j][i] - random.randint(0, lingyu)
                        if newGene[j][i] - lingyu > 0
                        else newGene[j][i]
                    )
                # if np.random.rand()>0.8:
    ...
    def get_fun3(self, gens):
        ...
        for i in range(self.partition_num):
            ...
            f3 = f3 + abs(
                V3_max - sum_V3
            )  # + abs(min(V3_max-sum_V3,0)*C) + abs(min(sum_V3-V3_min,0)*C)     # 负的罚函数
    ...
    def evolution(self):
        ...
        ax1.scatter3D(
            function1_values, function2_values, function3_values, cmap="Blues"
        )  # 绘制散点图
        plt.show()
        return function1_values, function2_values, function3_values


partition_num_max = int(
    sum(dingdan.loc[:num_loader]["坯料厚度"] * dingdan.loc[:num_loader]["数目"]) / 40000
)
partition_num_min = int(
    sum(dingdan.loc[:num_loader]["坯料厚度"] * dingdan.loc[:num_loader]["数目"]) / 60000
)
...
# 归一化
f1 = [
    (x - min(function1_values)) / (max(function1_values) - min(function1_values))
    for x in function1_values
]
f2 = [
    (x - min(function2_values)) / (max(function2_values) - min(function2_values))
    for x in function2_values
]
f3 = [
    (x - min(function3_values)) / (max(function3_values) - min(function3_values))
